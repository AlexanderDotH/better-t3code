...
- `GitWorkbenchDriver` constructs bounded Git commands and parses porcelain v2 NUL-delimited status.
- `GitWorkbenchService` validates registered workspaces, publishes detailed state, applies selections, and coordinates mutations.
- `GitRepositoryQueryService` owns bounded history, validated branch/path filters, commit detail, patch, rebase-plan, contributor, activity, and Code mix reads. The first history page resolves the chosen ref to a full object ID; every later cursor stays anchored to that snapshot.
- `GitWorkbenchOperations` serializes typed branch, reset, revert, cherry-pick, rebase, continuation, and force-with-lease actions.
