...
T3 gives Claude an authenticated `workspace_context` tool for batched project search and bounded
reads. Writable sessions also receive `workspace_edit` for single-file or mixed UTF-8 write,
...
writable session still lists the tool. Commands remain available under the normal permission policy
for unsupported file classes and generated changes.
