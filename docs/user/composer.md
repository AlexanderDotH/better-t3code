...
The first new message in a fork uses the same character and attachment limits as any other message.
Those limits apply only to the new message. T3 Code carries the complete frozen fork history and
all inherited attachments to the provider separately, without trimming older entries or counting
them against the new message.
...
Type `/` to open the command menu. Type `$` to find and add a skill. Skill rows show only catalog
metadata such as name, description, and source. Selecting a skill inserts its token; the provider
loads the full instructions through its native skill mechanism instead of T3 copying every
`SKILL.md` into each prompt.
