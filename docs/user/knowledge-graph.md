...
Open **Settings > Better T3 > Knowledge and automation**, then enable **Knowledge Graph**. T3 Code begins indexing every project registered in the connected environment, including known worktrees. Each worktree has its own graph.
...
- zoom and pan;
- drag nodes to arrange the visible view;
...
Reduced-motion preferences are respected. Large repositories show a bounded view first; search and expansion retrieve more of the persisted graph without trying to render the whole repository at once.
...
T3 Code watches eligible project files after the initial index. External edits are coalesced and applied as revisioned updates. Restarting the server recovers interrupted background enrichment work.
...
- **Cancel** stops the current indexing work without deleting the committed graph.
