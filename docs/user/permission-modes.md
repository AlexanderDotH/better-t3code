...
thread's mode; otherwise new threads start in **Full** unless you pick something else
before sending.
...
**Ruled**: auto-approve edits, ask before other actions. File changes go through
...
**Full**: allow commands and edits without prompts. The default. The agent runs
...
actions still ask for approval. It does not change the thread to **Full**.

## Choosing a Mode

Use **Full** for work in a worktree or a sandbox you can throw away.

Use **Supervised** on a repository where an unwanted command is expensive, or the first time you
run an unfamiliar task.

**Ruled** suits refactors where the edits are the point and you only care about the
shell commands.
...
with prompting enabled and a restricted workspace while **Full** disables both. Grok
threads do the same: **Supervised** starts Grok in ask mode even if your Grok CLI config is
set to always-approve, and **Full** starts Grok with always-approve. The labels above
