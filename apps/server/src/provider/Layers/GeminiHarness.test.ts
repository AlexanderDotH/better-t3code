...
const GEMINI_WORKSPACE_READ_TOOLS = [
  GEMINI_WORKSPACE_FIND_TOOL,
  GEMINI_WORKSPACE_READ_TOOL,
  GEMINI_WORKSPACE_CONTEXT_TOOL,
] as const;
...
describe("GeminiHarness", () => {
  it("keeps plan and read-only turns non-mutating", () => {
    ...
    ).toEqual([...GEMINI_WORKSPACE_READ_TOOLS]);
    expect(
    ...
    ).toEqual([...GEMINI_WORKSPACE_READ_TOOLS, GEMINI_WORKSPACE_EDIT_TOOL]);
    expect(
      geminiToolIsAvailable({
        toolName: "write_file",
  ...
  it("maps T3 runtime modes to edit and command approvals", () => {
    for (const name of GEMINI_WORKSPACE_READ_TOOLS) {
      expect(geminiToolRequiresApproval(name, "approval-required")).toBe(false);
    }
