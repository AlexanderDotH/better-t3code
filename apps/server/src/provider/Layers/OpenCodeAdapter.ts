...
export function makeOpenCodeAdapter(
  ...
  return Effect.gen(function* () {
    ...
    const awaitOpenCodeContextReady = Effect.fn("awaitOpenCodeContextReady")(function* (
      ...
      const current = yield* ensureSessionContext(provider, sessions, context.session.threadId);
      if (current !== context) {
        return yield* new ProviderAdapterSessionClosedError({
          provider,
    ...
    const reconcileOpenCodeIdle = Effect.fn("reconcileOpenCodeIdle")(function* (
      context: OpenCodeSessionContext,
      turnId: TurnId,
      raw: unknown,
    ) {
    ...
    const handleSubscribedEvent = Effect.fn("handleSubscribedEvent")(function* (
      ...
      switch (event.type) {
        ...
        case "permission.asked": {
          const openPermission = emitPendingOpenCodeRequest(context, event, event);
        ...
        case "session.status": {
          ...
          if (event.properties.status.type === "idle" && turnId) {
            yield* reconcileOpenCodeIdle(context, turnId, event);
          }
          break;
        }
        ...
        case "session.idle": {
          if (turnId) {
            ...
            if (admission?.turnId === turnId && admission.accepted) {
              ...
              yield* completeOpenCodeTurn(context, turnId, admission.generation, event);
              break;
            }
            yield* reconcileOpenCodeIdle(context, turnId, event);
          }
      ...
      return yield* context.promptSemaphore.withPermit(
        Effect.gen(function* () {
          ...
          const promptEffect = runOpenCodeSdk("session.promptAsync", (signal) =>
            ...
            Effect.timeout("10 seconds"),
            Effect.catchTags({
              OpenCodeRuntimeError: (cause) => Effect.fail(toRequestError(provider, cause)),
              TimeoutError: (cause) => {
                promptTimedOut = true;
                return Effect.fail(
                  new ProviderAdapterRequestError({
