...
describe("Codex subagent event mapping", () => {
  ...
  it("maps child turn failure to the child without producing a root turn completion", () => {
    ...
    NodeAssert.deepStrictEqual(
      events.map((event) => event.type),
      ["subagent.discovered", "subagent.state.changed", "turn.completed"],
    );
    ...
    NodeAssert.equal(events[2]?.subagentId, childId);
  });
...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  it.effect("force-stops the owned Codex runtime without cooperative close", () =>
    Effect.gen(function* () {
  ...
  it.effect("reports an already-absent Codex runtime as stopped", () =>
    Effect.gen(function* () {
      const adapter = yield* CodexAdapter;
