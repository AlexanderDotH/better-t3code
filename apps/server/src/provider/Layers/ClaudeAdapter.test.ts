...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("emits task_updated state before the terminal task notification", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      ...
      const nextEventsFiber = yield* Stream.take(adapter.streamEvents, 3).pipe(
        Stream.runCollect,
        Effect.forkChild,
      );
      ...
      assert.deepEqual(
        ...
        ["thread.started", "task.updated", "task.completed"],
      );
      const updated = nextEvents[1];
      assert.equal(updated?.type, "task.updated");
      if (updated?.type === "task.updated") {
        assert.equal(updated.payload.status, "failed");
        assert.equal(updated.payload.error, "Agent output exceeded the configured token limit.");
      }
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
      Effect.provide(harness.layer),
