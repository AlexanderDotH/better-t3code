...
export const GEMINI_WORKSPACE_FIND_TOOL = NATIVE_HARNESS_WORKSPACE_FIND_TOOL;
export const GEMINI_WORKSPACE_READ_TOOL = NATIVE_HARNESS_WORKSPACE_READ_TOOL;
export const GEMINI_WORKSPACE_CONTEXT_TOOL = NATIVE_HARNESS_WORKSPACE_CONTEXT_TOOL;
export const GEMINI_WORKSPACE_EDIT_TOOL = NATIVE_HARNESS_WORKSPACE_EDIT_TOOL;
export const GEMINI_EXEC_COMMAND_TOOL = NATIVE_HARNESS_EXEC_COMMAND_TOOL;

const GEMINI_ADVERTISED_TOOL_NAMES = new Set([
  GEMINI_WORKSPACE_FIND_TOOL,
  GEMINI_WORKSPACE_READ_TOOL,
...
export function geminiToolDeclarations(input: {
  ...
  return nativeHarnessToolDeclarations(input)
    .filter((declaration) => GEMINI_ADVERTISED_TOOL_NAMES.has(declaration.name))
    .map((declaration) => ({
...
export function geminiToolIsAvailable(input: {
  ...
  return GEMINI_ADVERTISED_TOOL_NAMES.has(input.toolName) && nativeHarnessToolIsAvailable(input);
}
