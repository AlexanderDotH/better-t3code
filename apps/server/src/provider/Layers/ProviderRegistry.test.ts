...
const GeminiWorkspaceLayer = Layer.merge(
  ...
  Layer.succeed(
    ...
    WorkspaceFileSystem.WorkspaceFileSystem.of({
      ...
      editFiles: () => Effect.die("unused Gemini workspace edit in provider registry test"),
    }),
  ),
);
  ...
  return Effect.gen(function* () {
    ...
    return {
      ...
      modifySettings: (modify) =>
        Effect.gen(function* () {
          const next = yield* modify(yield* Ref.get(settingsRef));
...
it.layer(
  Layer.mergeAll(
    ProviderRegistryTestLayerWithGeminiWorkspaceContextAndFileSystem,
    NoOpMcpConfigEngineLayer,
    ProviderDriverIntegrationTestLayer,
  ),
)("ProviderRegistry", (it) => {
    ...
    it.effect("refreshes OpenCode catalogs and preserves other providers", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          assert.strictEqual(
            initialProviders.find((provider) => provider.instanceId === openCodeInstanceId)?.status,
            "error",
          );
