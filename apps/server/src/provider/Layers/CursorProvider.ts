...
/**
 * Background enrichment for a Cursor snapshot.
 *
 * `cursor/list_available_models` is the primary model source. This enrichment
 * keeps version advisory updates and falls back to the slower per-model ACP
 * capability probe only when the snapshot still has uncaptured model options.
 */
