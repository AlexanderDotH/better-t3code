...
it.effect("force-stops at 5000ms even when cooperative interrupt never returns", () =>
  Effect.gen(function* () {
    ...
    yield* Effect.gen(function* () {
      ...
      const session = yield* Ref.get(harness.sessionRef);
      assert.strictEqual(session?.status, "stopped");
      assert.strictEqual(session?.abortState, null);
    }).pipe(Effect.provide(harness.layer));
  }),
);

it.effect("arms the watchdog while provider startup has a lease but no projected session yet", () =>
  Effect.gen(function* () {
    ...
    yield* Effect.gen(function* () {
      ...
      assert.strictEqual(stopping?.runtimeSessionId, runtimeSessionId);
...
it.effect("matching cooperative settlement cancels force escalation", () =>
  Effect.gen(function* () {
    ...
    yield* Effect.gen(function* () {
      ...
      yield* coordinator.requestAbort({ threadId, turnId, requestedAt });
      yield* Effect.yieldNow;
      yield* Ref.update(harness.sessionRef, (session): OrchestrationSession | null =>
        session === null
          ? null
          : {
              ...session,
...
it.effect("ignores cooperative terminal events from a stale runtime generation", () =>
  Effect.gen(function* () {
    const harness = yield* makeHarness();
    yield* Effect.gen(function* () {
      const coordinator = yield* TurnAbortCoordinator;
      ...
      assert.strictEqual(
        yield* coordinator.settleCooperative({
          threadId,
          runtimeSessionId: RuntimeSessionId.make("runtime-stale"),
          turnId,
          settledAt: "1970-01-01T00:00:03.000Z",
        }),
        false,
      );
      assert.notStrictEqual((yield* Ref.get(harness.sessionRef))?.abortState, null);
    }).pipe(Effect.provide(harness.layer));
  }),
);
