...
const make = Effect.gen(function* () {
  ...
  const commandId = (tag: string) =>
    crypto.randomUUIDv4.pipe(
      Effect.map((uuid) => CommandId.make(`server:turn-abort:${tag}:${uuid}`)),
    );
  ...
  const resolveSession = (threadId: ThreadId) =>
    resolveThread(threadId).pipe(Effect.map((thread) => thread?.session ?? null));

  const updateSession = (session: OrchestrationSession, createdAt: string) =>
    commandId("session-set").pipe(
  ...
  const cleanupAttempt = (attempt: AbortAttempt | undefined) =>
    attempt === undefined
      ? Effect.void
      : Effect.forEach(
          attempt.fibers,
  ...
  const removeAttempt = (target: ProviderAbortTarget) =>
  ...
  const watchdog = (target: ProviderAbortTarget) =>
    Effect.sleep(TURN_ABORT_FORCE_DELAY).pipe(
      Effect.andThen(
        lock.withPermits(1)(
          Effect.gen(function* () {
            ...
            if (
              session === null ||
              !sessionTargetsActiveAttempt(session, currentAttempt) ||
              !providerTargetIsCurrent
            ) {
              const removed = yield* removeAttempt(target);
              yield* cleanupAttempt(removed);
              return;
            }
