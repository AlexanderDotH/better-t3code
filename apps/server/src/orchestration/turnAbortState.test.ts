...
it.layer(NodeServices.layer)("turn abort orchestration state", (it) => {
  it.effect("decides settlement only for the exact runtime and turn", () =>
    Effect.gen(function* () {
  ...
  it.effect("clears abort state while keeping a cooperatively stopped runtime resumable", () =>
    Effect.gen(function* () {
      const readModel = yield* seedRunningAbort;
  ...
  it.effect("stops a forced runtime and ignores stale settlement events", () =>
    Effect.gen(function* () {
      const readModel = yield* seedRunningAbort;
