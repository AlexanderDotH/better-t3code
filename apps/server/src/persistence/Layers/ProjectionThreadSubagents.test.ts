...
layer("ProjectionThreadSubagent repositories", (it) => {
  ...
  it.effect("deletes all subagent projections for one root thread", () =>
    Effect.gen(function* () {
      const summaries = yield* ProjectionThreadSubagentRepository;
      const messages = yield* ProjectionThreadSubagentMessageRepository;
      const proposedPlans = yield* ProjectionThreadSubagentProposedPlanRepository;
      const activities = yield* ProjectionThreadSubagentActivityRepository;
      const threadId = ThreadId.make("thread-delete-root");
      const subagentId = SubagentId.make("agent-delete-root");
      const timestamp = "2026-07-30T12:00:00.000Z";

      yield* summaries.upsert({
        threadId,
        id: subagentId,
        providerThreadId: "provider-delete-root",
        parentId: null,
        path: "/root/delete",
        name: "delete-root",
        nickname: null,
        role: null,
        task: "Delete root projections",
        model: null,
      ...
      yield* messages.upsert({
        threadId,
        subagentId,
        messageId: MessageId.make("message-delete-root"),
        turnId: null,
        role: "assistant",
        text: "Delete me",
        isStreaming: false,
      ...
      yield* proposedPlans.upsert({
        threadId,
        subagentId,
        planId: OrchestrationProposedPlanId.make("plan-delete-root"),
        turnId: null,
        planMarkdown: "# Delete me",
        implementedAt: null,
        implementationThreadId: null,
        createdAt: timestamp,
        updatedAt: timestamp,
      ...
      yield* activities.upsert({
        threadId,
        subagentId,
        activityId: EventId.make("activity-delete-root"),
        turnId: null,
        tone: "info",
        kind: "cleanup",
        summary: "Delete me",
      ...
      yield* proposedPlans.deleteByThreadId({ threadId });
      yield* activities.deleteByThreadId({ threadId });
      yield* summaries.deleteByThreadId({ threadId });

      assert.deepStrictEqual(yield* summaries.listByThreadId({ threadId }), []);
      assert.deepStrictEqual(yield* messages.listBySubagentId({ threadId, subagentId }), []);
      assert.deepStrictEqual(yield* proposedPlans.listBySubagentId({ threadId, subagentId }), []);
      assert.deepStrictEqual(yield* activities.listBySubagentId({ threadId, subagentId }), []);
    }),
  );
});
