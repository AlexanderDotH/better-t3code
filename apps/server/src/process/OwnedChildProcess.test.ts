...
describe("forceTerminateOwnedChildProcess", () => {
  it.effect("does not signal an exact child that has already exited", () =>
    Effect.gen(function* () {
      let killCount = 0;
  ...
  it.effect("uses SIGKILL and verifies that the captured child exited", () =>
    Effect.gen(function* () {
      let running = true;
  ...
  it.effect("fails when SIGKILL returns but the exact child is still running", () =>
    Effect.gen(function* () {
      const child = makeHandle({
      ...
      const error = yield* forceTerminateOwnedChildProcess({ child }).pipe(Effect.flip);
