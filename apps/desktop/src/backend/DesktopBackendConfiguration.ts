...
> {
  ...
  return {
    ...
    linuxAppRoot: mounted.linuxAppRoot,
...
> {
  ...
  const wslResourceMonitorPath =
    preflight._tag === "Failed"
      ? Option.none<string>()
      : yield* (
          environment.isPackaged
            ? wslEnvironment
                .prepareResourceMonitor(
                  preflight.runningDistro,
                  `${preflight.linuxAppRoot}/${WSL_RESOURCE_MONITOR_DIRECTORY}/t3-resource-monitor`,
                )
                .pipe(
                  Effect.map((ready) =>
                    ready
            ...
            : resolveWslResourceMonitorLinuxPath({
                distro: preflight.runningDistro,
                wslAppRoot: environment.rootDir,
              })
        ).pipe(
