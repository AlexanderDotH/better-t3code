...
const TEST_SESSION_EXPIRES_AT = DateTime.makeUnsafe("2030-05-01T12:00:00.000Z");
...
export function createAuthenticatedSessionHandlers(
  ...
  getEnvironmentDescriptor: (requestUrl: string) => ExecutionEnvironmentDescriptor = () =>
    TEST_ENVIRONMENT_DESCRIPTOR,
) {
  ...
  const metadataLayer = HttpApiBuilder.group(BrowserEnvironmentHttpApi, "metadata", (handlers) =>
    handlers.handle("descriptor", () =>
      Effect.gen(function* () {
        ...
        return getEnvironmentDescriptor(request.url);
      }),
    ),
  );
  const authLayer = HttpApiBuilder.group(BrowserEnvironmentHttpApi, "auth", (handlers) =>
    handlers
      ...
      .handle("webSocketTicket", () =>
        Effect.succeed({
          ticket: "browser-websocket-ticket",
          expiresAt: TEST_SESSION_EXPIRES_AT,
        }),
      )
