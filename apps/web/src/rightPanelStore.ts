...
export function migratePersistedRightPanelState(persistedState: unknown): {
  ...
  const byThreadKey =
    persistedByThreadKey && typeof persistedByThreadKey === "object"
      ? Object.fromEntries(
          Object.entries(persistedByThreadKey as Record<string, ThreadRightPanelState>)
            ...
            .map(([threadKey, threadState]) => {
              ...
              const requestedActiveSurfaceId =
                typeof validThreadState?.activeSurfaceId === "string"
                  ...
                  : null;
              const preservedActiveSurface = surfaces.find((surface) =>
                requestedActiveSurfaceId === "pull-request"
                  ? surface.kind === "pull-request"
                  : surface.id === requestedActiveSurfaceId,
              );
