...
describe("Git workspace deck motion CSS", () => {
  ...
  it.effect("fades content quickly around a delayed, slower vertical shuffle", () =>
    Effect.gen(function* () {
      const css = yield* readWorkspaceDeckCss;
      ...
      expect(css).toContain("@keyframes workspace-card-to-front");
      expect(css).toContain("@keyframes workspace-card-to-back");
      expect(css).toContain("animation: workspace-card-to-front 600ms");
      expect(css).toContain("animation: workspace-card-to-back 600ms");
      ...
      expect(css).not.toContain("--workspace-card-shuffle-in-x");
      expect(css).not.toContain("--workspace-card-shuffle-out-x");
      expect(css).not.toContain("rotate(");
      expect(cardKeyframes).not.toContain("opacity:");
      expect(css).toMatch(/@keyframes workspace-card-to-front[\s\S]*?43%\s*\{[^}]*z-index:\s*4;/);
      expect(css).toMatch(
  ...
  it.effect(
    "finishes the outgoing fade before motion and restores incoming content mid-shuffle",
    () =>
      Effect.gen(function* () {
        const css = yield* readWorkspaceDeckCss;

        expect(css).toContain("@keyframes workspace-content-to-back");
        expect(css).toContain("@keyframes workspace-content-to-front");
        expect(css).toMatch(/workspace-content-to-back 90ms[^;]*both/);
        expect(css).toMatch(/workspace-content-to-front 300ms[^;]*350ms both/);
        expect(css).toMatch(
          /@keyframes workspace-content-to-back[\s\S]*?100%\s*\{[^}]*opacity:\s*0;/,
        );
        expect(css).toMatch(
          /@keyframes workspace-content-to-front[\s\S]*?0%\s*\{[^}]*opacity:\s*0;[\s\S]*?100%\s*\{[^}]*opacity:\s*1;/,
        );
      }),
  );
  ...
  it.effect("swaps and resizes immediately when reduced motion is requested", () =>
    Effect.gen(function* () {
      ...
      expect(css).toMatch(
        /@media \(prefers-reduced-motion: reduce\)[\s\S]*?\.workspace-card-deck__card-content[\s\S]*?animation:\s*none;/,
      );
    }),
  );
});
