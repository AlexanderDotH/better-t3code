...
export function resolveManualCompactAction(input: {
  readonly provider: string;
  readonly canManualCompact: boolean;
  readonly compactClaude: () => void;
  readonly compactProvider?: (() => void) | undefined;
}): (() => void) | undefined {
  if (input.provider === "claudeAgent") return input.compactClaude;
  if (!input.canManualCompact) return undefined;
  return input.compactProvider;
}
