...
export const SUBAGENT_ARCHIVE_AFTER_MS = SUBAGENT_TERMINAL_COLOR_MS + SUBAGENT_STALE_MS;

const ACTIVE_STATUSES = new Set<OrchestrationSubagentStatus>(["starting", "running", "waiting"]);

export type SubagentLifecycleStage = "working" | "success" | "failure" | "stale" | "archived";

export interface SubagentLifecycleEntry {
  readonly agent: OrchestrationSubagentSummary;
  readonly stage: Exclude<SubagentLifecycleStage, "archived">;
...
function lifecycleEntry(
  ...
  const terminalStage = agent.status === "completed" ? "success" : "failure";
  ...
  const staleAtMs = terminalAtMs + SUBAGENT_TERMINAL_COLOR_MS;
  const archiveAtMs = terminalAtMs + SUBAGENT_ARCHIVE_AFTER_MS;
