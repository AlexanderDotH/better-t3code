...
export const SUBAGENT_ENTER_MS = 360;
export const SUBAGENT_EXIT_MS = 260;
export const SUBAGENT_SLOT_COLLAPSE_MS = 180;

const MAX_TIMEOUT_MS = 2_147_483_647;

export type SubagentPresencePhase = "present" | "entering" | "exiting" | "collapsing";
...
export function reconcileSubagentPresence(
  ...
  const reconciled: SubagentPresenceEntry[] = desired.map((entry) => {
    ...
    if (existing.phase === "exiting" || existing.phase === "collapsing") {
      return {
        ...entry,
        phase: "entering",
        phaseStartedAtMs: nowMs,
  ...
  for (const entry of current) {
    ...
    if (entry.phase === "exiting" || entry.phase === "collapsing") {
