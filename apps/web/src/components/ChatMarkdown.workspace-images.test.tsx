...
const testState = vi.hoisted(() => ({
  ...
  locale: { language: "en", locale: "en-US" } as ResolvedInterfaceLocale,
}));
...
vi.mock("../interfaceLanguageRuntime", () => ({
  useInterfaceLocaleRuntime: () => testState.locale,
}));
...
describe("ChatMarkdown workspace images", () => {
  beforeEach(() => {
    ...
    testState.locale = { language: "en", locale: "en-US" };
  });
  ...
  it("localizes workspace image loading and failure states", () => {
    testState.assetState = "loading";
    testState.locale = { language: "de", locale: "de-DE" };
    expect(render("![diagram](images/diagram.png)")).toContain('aria-label="Bild wird geladen"');

    testState.assetState = "failure";
    testState.locale = { language: "fr", locale: "fr-FR" };
    expect(render("![diagram](images/diagram.png)")).toContain("Image indisponible · diagram");
  });
