...
export const ChatAgentStack = memo(function ChatAgentStack({
  ...
  return (
    <div
      ...
      className={cn("pointer-events-none w-52 max-w-[calc(100vw-1.5rem)]", className)}
    >
      <ScrollArea
        className="h-auto max-h-[min(20rem,calc(100vh-6rem))] w-full"
        hideScrollbars
        scrollFade
      >
      ...
      <TooltipTrigger
        render={
          <button
            ...
            className={cn(
              "subagent-stack-pill group flex h-7 w-fit max-w-52 items-center gap-1.5 rounded-full border border-border/65 bg-background/88 px-2.5 text-xs font-medium text-foreground shadow-md/8 outline-none backdrop-blur-xl transition-[background-color,border-color,box-shadow] duration-200",
              "pointer-events-auto",
