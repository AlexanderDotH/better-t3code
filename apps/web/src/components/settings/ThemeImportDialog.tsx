...
function ThemeJsonEditor({
  ...
  return (
    <div className="relative overflow-hidden rounded-xl border border-input bg-background shadow-xs/5 focus-within:border-ring focus-within:ring-[3px] focus-within:ring-ring/24">
      ...
      <textarea
        aria-label={translate("settings.theme.import.json")}
        className={cn(
  ...
  const handleSubmit = useCallback(() => {
    ...
    try {
      ...
      if (!onImported(installedTheme)) {
        ...
        setError(translate("settings.theme.import.selectFailed"));
        return;
      }
      onOpenChange(false);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : translate("settings.theme.import.invalid"));
    }
  }, [json, onImported, onOpenChange, translate]);
  ...
  return (
    ...
    >
      <DialogPopup className="max-w-3xl overflow-hidden">
        <DialogHeader>
          <DialogTitle>{translate("settings.theme.import.title")}</DialogTitle>
        </DialogHeader>
        <DialogPanel className="space-y-5">
          ...
          <div className="flex items-center gap-3" aria-hidden>
            ...
            <span className="text-muted-foreground text-[11px] uppercase tracking-wider">
              {translate("settings.theme.import.orFile")}
            </span>
            <div className="h-px flex-1 bg-border" />
          </div>
          ...
          {(() => {
            ...
            const chooseButton = (label = translate("settings.theme.import.chooseFiles")) => (
              <Button disabled={isReading} size="sm" variant="outline" onClick={openFilePicker}>
                <DownloadIcon />
                {isReading ? translate("settings.theme.import.reading") : label}
              </Button>
            );
            const editorSection = () => (
              <div className="space-y-2">
                <div className="flex items-baseline justify-between gap-3">
                  <label className="text-sm font-medium" htmlFor="theme-json-editor">
                    {translate("settings.theme.import.json")}
                  </label>
                </div>
                ...
                <div className="space-y-3">
                  <div className="rounded-xl border border-border/70 bg-muted/20 p-3">
                    <p className="text-sm font-medium">
                      {translate("settings.theme.import.installed")}
                    </p>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <Button size="sm" onClick={() => resolveConflicts("update")}>
                      {translate("settings.theme.import.updateExisting")}
                    </Button>
                    <Button size="sm" variant="outline" onClick={() => resolveConflicts("copy")}>
                      {translate("settings.theme.import.keepBoth")}
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => setConflicts(null)}>
                      {translate("settings.theme.import.back")}
                    </Button>
              ...
              <div className="space-y-4">
                ...
                >
                  <div className="min-w-0">
                    <p className="text-sm font-medium">{translate("settings.theme.import.file")}</p>
                    <p className="truncate text-xs text-muted-foreground">
                      {fileName ?? translate("settings.theme.import.dropFiles")}
                    </p>
                  </div>
                  {chooseButton()}
                  {fileInput}
                ...
                <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                  ...
                  <Button disabled={!json.trim() || isReading} onClick={handleSubmit}>
                    <PlusIcon />
                    {translate("settings.theme.import.add")}
                  </Button>
                </div>
              </div>
            );
