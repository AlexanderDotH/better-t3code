  ...
  if (lastCheckedRelative.status === "invalid") {
    return (
      <span className="text-[11px] text-muted-foreground/50">
        {translate("settings.providers.panel.checkedUnavailable")}
      </span>
    );
  }
    ...
    <span className="text-[11px] text-muted-foreground/60">
      {translate(
        lastCheckedRelative.suffix
          ...
          : "settings.providers.panel.checked",
        { time: lastCheckedRelative.value },
      )}
    </span>
  );
}
...
function providerEnvironmentDetail(
  ...
  if (environment.entry.target._tag === "PrimaryConnectionTarget") {
    return translate("settings.providers.panel.primaryDevice");
  }
  ...
  if (isDesktopLocalConnectionTarget(environment.entry.target)) {
    return translate("settings.providers.panel.localDevice");
  }
  return environment.displayUrl ?? translate("settings.providers.panel.remoteDevice");
}
...
function EnvironmentUnavailableRow({
  ...
  const title = isLoading
    ? translate("settings.providers.panel.loading")
    : access.kind === "error"
      ? translate("settings.providers.panel.connectFailed")
      : translate("settings.providers.panel.unavailable");
  const description = isLoading
    ? access.reason === "permissions"
      ? translate("settings.providers.panel.checkingPermissions")
      : translate("settings.providers.panel.waitingConfiguration", {
  ...
  return (
    <SettingsSection title={translate("settings.providers.panel.title")}>
      {deviceTabs}
  ...
  const deviceTabs =
    !onlyPrimaryDevice && options.length > 0 ? (
      <ScrollArea hideScrollbars scrollFade className="mx-3 h-11 min-w-0 rounded-none sm:mx-4">
        <div
          role="group"
          aria-label={translate("settings.providers.panel.devices")}
        ...
        >
          {options.map((environment) => {
            ...
            const detail = providerEnvironmentDetail(environment, translate);
...
export function EnvironmentProviderSettings({
  ...
  const runProviderUpdate = useCallback(
    async (candidate: ProviderUpdateCandidate) => {
      ...
      if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
        ...
        toastManager.add(
          stackedThreadToast({
            type: "error",
            title: translate("settings.providers.panel.updateFailed", {
              provider: PROVIDER_DISPLAY_NAMES[candidate.driver] ?? candidate.driver,
            }),
            description:
              error instanceof Error
                ? error.message
                : translate("settings.providers.panel.updateStartFailed"),
          }),
        );
      }
    ...
    [environmentId, translate, updateProvider],
  );
  ...
  const deleteProviderInstance = (id: ProviderInstanceId) => {
    updateSettings(buildDeleteProviderInstancePatch(settings, id));
  };
  ...
  const resetDefaultInstance = (driverKind: ProviderDriverKind) => {
    const patch = buildResetDefaultProviderInstancePatch(settings, driverKind);
      ...
      <ProviderInstanceCard
        ...
        headerAction={
          mode === "editor" && row.isDefault && row.isDirty ? (
            <SettingResetButton
              label={translate("settings.providers.panel.resetLabel", {
                provider: resetLabel,
              })}
  ...
  return (
    <>
            ...
            {!readOnly ? (
              <>
                <Tooltip>
                  <TooltipTrigger
                    render={
                      <Button
                        ...
                        aria-label={translate("settings.providers.panel.refresh")}
                      >
                  ...
                  />
                  <TooltipPopup side="top">
                    {translate("settings.providers.panel.refresh")}
                  </TooltipPopup>
                </Tooltip>
                <Tooltip>
                  <TooltipTrigger
                    render={
                      <Button
                        ...
                        aria-label={translate("settings.providers.panel.add")}
                      >
                  ...
                  />
                  <TooltipPopup side="top">
                    {translate("settings.providers.panel.add")}
                  </TooltipPopup>
                </Tooltip>
              </>
            ) : null}
      ...
      >
        ...
        {readOnly ? (
          <SettingsRow
            title={translate("settings.providers.panel.limited")}
            description={translate("settings.providers.panel.readOnly", {
        ...
        <div className="space-y-1">
          <div className="mx-3 overflow-hidden rounded-lg border border-border/70 sm:mx-4 lg:grid lg:h-[min(38rem,calc(100dvh-16rem))] lg:min-h-[30rem] lg:grid-cols-[20rem_minmax(0,1fr)]">
            <div className="border-b border-border/70 lg:flex lg:min-h-0 lg:flex-col lg:border-r lg:border-b-0">
              <ScrollArea scrollFade chainVerticalScroll className="lg:min-h-0 lg:flex-1">
                <div data-provider-group="core" className="divide-y divide-border/60">
                  {providerGroups.core.map((row) => (
                ...
                {providerGroups.additional.length > 0 ? (
                  <div data-provider-group="better-t3" className="border-t border-border/70">
                    <div className="px-3 py-2 text-[11px] font-medium text-muted-foreground">
                      {interfaceTranslator.message("settings.betterT3.providers.additionalHeading")}
                    </div>
                    <div className="divide-y divide-border/60 border-t border-border/60">
                      {providerGroups.additional.map((row) => (
                        <div key={row.instanceId} className="p-1">
                          {renderProviderInstance(row, "list")}
                        </div>
                      ))}
            ...
            <div className="min-w-0 lg:min-h-0">
              {selectedRow ? (
                ...
                <div className="p-6 text-sm text-muted-foreground">
                  {translate("settings.providers.panel.noneConfigured")}
                </div>
              )}
            </div>
          </div>
          ...
          <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen} className="mt-1">
            <CollapsibleTrigger className="flex h-10 w-full items-center gap-2 px-3 text-xs text-muted-foreground hover:text-foreground sm:px-4">
              ...
              />
              {translate("settings.providers.panel.advanced")}
            </CollapsibleTrigger>
            <CollapsibleContent>
              <div
                inert={readOnly}
                ...
                <SettingsRow
                  title={
                    <span className="inline-flex items-center gap-1.5">
                      {translate("settings.providers.panel.healthInterval")}
                      <PolicyTooltip>
                        {translate("settings.providers.panel.healthPolicy")}
                      </PolicyTooltip>
                    </span>
                  }
                  description={translate("settings.providers.panel.healthDescription")}
                  resetAction={
                    providerHealthRefreshIntervalSeconds !==
                    defaultProviderHealthRefreshIntervalSeconds ? (
                      <SettingResetButton
                        label={translate("settings.providers.panel.healthReset")}
                        onClick={() =>
                          updateSettings(
                            backgroundActivityOverrideSettings(
                  ...
                  control={
                    <div className="flex shrink-0 items-center gap-2">
                      ...
                      >
                        <NumberFieldGroup>
                          <NumberFieldDecrement
                            aria-label={translate("settings.providers.panel.healthDecrease")}
                          />
                          <NumberFieldInput
                            aria-label={translate("settings.providers.panel.healthInput")}
                          />
                          <NumberFieldIncrement
                            aria-label={translate("settings.providers.panel.healthIncrease")}
                          />
                        </NumberFieldGroup>
                      </NumberField>
                      <span className="text-xs text-muted-foreground">
                        {translate("settings.providers.panel.seconds")}
                      </span>
                    </div>
                  }
                />
