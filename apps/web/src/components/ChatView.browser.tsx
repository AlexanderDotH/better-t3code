...
const SECONDARY_THREAD_ID = "thread-secondary-project" as ThreadId;
...
const BASE_TIME_MS = Date.now() - 60 * 60 * 1_000;
...
function createSnapshotWithOlderSecondaryProject(): OrchestrationReadModel {
  const snapshot = createSnapshotWithSecondaryProject({ includeArchivedSecondaryThread: false });
  const olderAt = new Date(BASE_TIME_MS - SIDEBAR_PROJECT_INACTIVITY_MS - 60_000).toISOString();

  return {
    ...snapshot,
    projects: snapshot.projects.map((project) =>
      project.id === SECOND_PROJECT_ID
        ? { ...project, createdAt: olderAt, updatedAt: olderAt }
        : project,
    ),
    threads: snapshot.threads.map((thread) =>
      thread.id === SECONDARY_THREAD_ID
        ? {
            ...
            createdAt: olderAt,
            updatedAt: olderAt,
            session: thread.session ? { ...thread.session, updatedAt: olderAt } : null,
          }
        : thread,
    ),
  };
...
describe("ChatView timeline estimator parity (full app)", () => {
  ...
  it("moves inactive projects into the collapsed older-projects section", async () => {
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotWithOlderSecondaryProject(),
    });
  ...
  it("reveals an active older project without persisting the disclosure", async () => {
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotWithOlderSecondaryProject(),
      initialPath: `/${LOCAL_ENVIRONMENT_ID}/${SECONDARY_THREAD_ID}`,
    });
