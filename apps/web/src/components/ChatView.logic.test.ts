...
describe("resolveKnowledgeGraphSurfacePolicy", () => {
  it("keeps the existing owner open while feature-off hides new launch affordances", () => {
    expect(
      resolveKnowledgeGraphSurfacePolicy({
        hasProject: true,
    ...
    ).toEqual({ ownerAvailable: true, launchAvailable: false, closeExisting: false });
    ...
    ).toEqual({ ownerAvailable: true, launchAvailable: true, closeExisting: false });
  });

  it("closes only when the owner is unsupported or has no project context", () => {
    expect(
      resolveKnowledgeGraphSurfacePolicy({
        hasProject: true,
        knowledgeGraphVersion: 0,
        enabled: true,
      }),
    ).toEqual({ ownerAvailable: false, launchAvailable: false, closeExisting: true });
    expect(
      resolveKnowledgeGraphSurfacePolicy({
        hasProject: false,
        knowledgeGraphVersion: 1,
...
describe("resolveSpeechEnvironmentId", () => {
  it("routes voice operations through the active thread environment", () => {
    const remoteEnvironmentId = EnvironmentId.make("environment-remote");

    expect(resolveSpeechEnvironmentId(environmentId, remoteEnvironmentId)).toBe(
      remoteEnvironmentId,
    );
  });

  it("falls back to the route environment before a thread environment is available", () => {
    expect(resolveSpeechEnvironmentId(environmentId, undefined)).toBe(environmentId);
  });
});
