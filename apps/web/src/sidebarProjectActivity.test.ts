...
describe("partitionSidebarProjectsByActivity", () => {
  it("moves exactly seven days to older and keeps one millisecond newer recent", () => {
    ...
    const oneMillisecondRecent = makeProject("recent", [
      makeMember("recent", {
    ...
    const result = partition([exactlySevenDays, oneMillisecondRecent]);

    expect(result.recentProjects.map((project) => project.projectKey)).toEqual(["recent"]);
    expect(result.olderProjects.map((project) => project.projectKey)).toEqual(["exact"]);
  });
  ...
  it("uses latest user activity instead of recent project or thread metadata", () => {
    const oldTimestamp = timestampAtAge(SIDEBAR_PROJECT_INACTIVITY_MS);
  ...
  it("keeps every attention state in the recent list regardless of age", () => {
    const oldTimestamp = timestampAtAge(SIDEBAR_PROJECT_INACTIVITY_MS + 1);
    const projects = ["approval", "input", "plan", "session", "turn"].map((key) =>
  ...
  it("clamps future activity so it can age out after seven days", () => {
    ...
    expect(result.nextTransitionAtMs).toBe(NOW_MS + SIDEBAR_PROJECT_INACTIVITY_MS);
  });
  ...
  it("returns the earliest upcoming transition and excludes attention-held projects", () => {
    ...
    expect(result.nextTransitionAtMs).toBe(NOW_MS + 500);
  });
});
