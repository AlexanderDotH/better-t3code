...
export function BrowserDeviceToolbar({
  ...
  const selectedValue =
    ...
    PREVIEW_VIEWPORT_PRESETS.some((preset) => preset.id === setting.presetId)
      ...
      : RESPONSIVE_VALUE;
  const selectItems = [
    { value: RESPONSIVE_VALUE, label: translate("browser.device.responsive") },
  ...
  return (
    <div
      ...
      role="toolbar"
      aria-label={translate("browser.device.toolbar")}
    ...
    >
      ...
      >
        ...
        <SelectPopup align="start" alignItemWithTrigger={false} className="min-w-64">
          <SelectItem value={RESPONSIVE_VALUE}>{translate("browser.device.responsive")}</SelectItem>
          <SelectGroup>
            <SelectGroupLabel>{translate("browser.device.standard")}</SelectGroupLabel>
      ...
      <form
        ...
        aria-label={translate("browser.device.viewportDimensions")}
        onSubmit={(event) => {
      ...
      <Tooltip>
        <TooltipTrigger
          render={
            <Button
              ...
              aria-label={
                aspectRatio === null
                  ? translate("browser.device.lockViewportAspectRatio")
        ...
        <TooltipPopup side="top">
          {aspectRatio === null
            ? translate("browser.device.lockAspectRatio")
            : translate("browser.device.unlockAspectRatio")}
        </TooltipPopup>
      </Tooltip>
      <Button
        variant="ghost"
        size="icon-xs"
        type="button"
        aria-label={translate("browser.device.rotate")}
        disabled={pending}
        onClick={rotate}
      >
        <ScreenRotationIcon />
