...
describe("mobile composer drafts", () => {
  ...
  it("drops persisted recommendation state", () => {
    const drafts = decodePersistedComposerDrafts({
      schemaVersion: 1,
      drafts: {
        "environment-1:thread-1": {
    ...
    expect(drafts["environment-1:thread-1"]?.text).toBe("keep this draft");
    expect(drafts["environment-1:thread-1"]).not.toHaveProperty("reasoningRecommendation");
  });
