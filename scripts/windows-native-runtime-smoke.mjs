...
async function runConPtyProbe(tempRoot) {
  ...
  await NodeFSP.writeFile(
    ...
    [
      ...
      "let output = '';",
      "terminal.onData((chunk) => { output += chunk; if (output.includes(token)) process.stdout.write(output, () => process.exit(0)); });",
      "terminal.onExit(({ exitCode }) => { if (!output.includes(token)) process.exit(exitCode || 1); });",
      "",
    ].join("\n"),
    "utf8",
  );
