...
it.effect("decodes a force-abort command for one explicit turn", () =>
  Effect.gen(function* () {
    const parsed = yield* decodeOrchestrationCommand({
      type: "thread.turn.force-abort",
      commandId: "command-force-abort-1",
      threadId: "thread-1",
      turnId: "turn-1",
      createdAt: "2026-07-30T10:00:00.000Z",
    });

    assert.strictEqual(parsed.type, "thread.turn.force-abort");
    if (parsed.type !== "thread.turn.force-abort") {
      return;
    }
...
it.effect("decodes a force-abort requested event", () =>
  Effect.gen(function* () {
    const parsed = yield* decodeOrchestrationEvent({
      eventId: "event-force-abort-1",
      commandId: "command-force-abort-1",
      aggregateKind: "thread",
      aggregateId: "thread-1",
      ...
      correlationId: "command-force-abort-1",
      causationEventId: null,
      metadata: {},
      sequence: 1,
    });

    assert.strictEqual(parsed.type, "thread.turn-force-abort-requested");
  }),
);
