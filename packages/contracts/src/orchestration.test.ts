...
it.effect("decodes subagent summary, state, and progress commands", () =>
  Effect.gen(function* () {
    ...
    const state = yield* decodeOrchestrationCommand({
    ...
    assert.strictEqual(state.type, "thread.subagent.state.set");
    assert.strictEqual(progress.type, "thread.subagent.progress.set");
  }),
);

it.effect("decodes routed subagent transcript events and lifecycle updates", () =>
  Effect.gen(function* () {
