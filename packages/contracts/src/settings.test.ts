...
describe("ClientSettings sidebar", () => {
  it("defaults to the current sidebar with automatic merge and inactivity settling", () => {
    ...
    expect(decodeServerSettings({}).sidebarAutoSettleAfterDays).toBe(3);
    expect(decodeServerSettings({}).sidebarAutoSettleOnMerge).toBe(true);
...
describe("ServerSettings native provider boundary", () => {
  it("hydrates defaults for all native providers", () => {
    const decoded = decodeServerSettings({});
