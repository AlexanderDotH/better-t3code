...
describe("requestPlanParallelismReview", () => {
  ...
  it("bounds cached plan versions and refreshes an evicted review", async () => {
    const review = vi.fn(
      async (
        ...
        recommendedSubagents: 4,
      }),
    );
    for (let index = 0; index < 51; index += 1) {
      await requestPlanParallelismReview(
        { ...descriptor, planUpdatedAt: `version-${index}` },
        review,
      );
    }
    await requestPlanParallelismReview({ ...descriptor, planUpdatedAt: "version-50" }, review);
    expect(review).toHaveBeenCalledTimes(51);
    await requestPlanParallelismReview({ ...descriptor, planUpdatedAt: "version-0" }, review);
    expect(review).toHaveBeenCalledTimes(52);
  });

  it("does not retain review keys exceeding the cache byte budget", async () => {
    const oversized = {
      ...descriptor,
      reviewerSelection: { ...reviewerSelection, model: "x".repeat(40_000) },
    };
    const review = vi.fn(async () => ({
      planId: descriptor.planId,
      planUpdatedAt: descriptor.planUpdatedAt,
      implementationProviderInstanceId,
      recommendedSubagents: 4,
    }));
    await requestPlanParallelismReview(oversized, review);
    await requestPlanParallelismReview(oversized, review);
