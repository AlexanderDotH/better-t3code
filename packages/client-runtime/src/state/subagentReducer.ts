...
function isRoutedToSubagent(event: OrchestrationEvent, subagentId: SubagentId): boolean {
  switch (event.type) {
...
export function applySubagentDetailEvent(
  ...
  if (event.type === "thread.subagent-state-set") {
    ...
    if (updated === undefined || updated === subagent) {
      return { kind: "unchanged" };
    }
    return {
      kind: "updated",
  ...
  if (!isRoutedToSubagent(event, subagent.id)) {
    return { kind: "unchanged" };
  }
