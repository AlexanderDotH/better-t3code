...
import { SubagentSnapshotLoader } from "./subagentSnapshotHttp.ts";

// Selected transcripts are intentionally in-memory and lazy. Retain one across
// short tab/layout changes, then close its subscription and release the data.
...
export const makeEnvironmentSubagentState = Effect.fn("EnvironmentSubagentState.make")(function* (
  ...
  yield* Effect.gen(function* () {
    ...
    const base = Option.isSome(prepared)
      ? yield* snapshotLoader.load(prepared.value, threadId, subagentId)
      : Option.none();
    ...
    yield* subscribe(ORCHESTRATION_WS_METHODS.subscribeSubagent, subscribeInput, {
