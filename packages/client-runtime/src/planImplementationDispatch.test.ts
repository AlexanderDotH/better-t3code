...
import { resolvePlanImplementationDispatch } from "./planImplementation.ts";
...
describe("resolvePlanImplementationDispatch", () => {
  ...
  it("uses only the exact selected provider-instance capability", () => {
    ...
    if (result._tag === "Ready") {
      ...
      expect(result.prompt).toContain("exactly 2 direct child subagents");
      expect(result.prompt).toContain("Subagents must not spawn additional agents");
    }
  });
