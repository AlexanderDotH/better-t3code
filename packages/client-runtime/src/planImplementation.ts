...
function parseHeading(line: string): { readonly level: number; readonly title: string } | null {
  ...
  const title = match?.[2];
  return marker && title ? { level: marker.length, title: normalizeSectionTitle(title) } : null;
}

function listItemIndent(line: string): number | null {
  const match = line.match(/^([ \t]*)(?:[-+*]|\d+[.)])[ \t]+(?:\[[ xX]\][ \t]+)?\S/);
  const indentation = match?.[1];
  if (indentation === undefined) return null;
...
export function estimatePlanImplementationWorkUnits(planMarkdown: string): number {
  ...
  for (const line of planMarkdown.split(/\r?\n/)) {
    ...
    if (heading) {
      activeListIndent = null;
      if (excludedSectionLevel !== null && heading.level > excludedSectionLevel) continue;
      excludedSectionLevel = null;
      if (EXCLUDED_SECTION_TITLES.has(heading.title)) {
        excludedSectionLevel = heading.level;
        continue;
      }
      if (heading.level === 2 || heading.level === 3) eligibleHeadingCount += 1;
      continue;
    }

    if (excludedSectionLevel !== null) continue;
    const indent = listItemIndent(line);
    if (indent === null) {
      if (line.trim().length > 0) activeListIndent = null;
      continue;
    }
    ...
    if (indent === activeListIndent) topLevelListItemCount += 1;
  }
...
function planSubagentCeiling(
  ...
  if (!capability || capability.toolName.trim().length === 0) return null;
...
export function getSupportedPlanSubagentCounts(
  ...
  return ceiling === null ? [] : Array.from({ length: ceiling - 1 }, (_, index) => index + 2);
}
...
export function resolvePlanImplementationSuggestion(input: {
  ...
  return { strategy: { kind: "subagents", count }, supportedCounts };
}
...
export function buildPlanImplementationPrompt(
  ...
  if (strategy.kind === "standard") return buildStandardPlanImplementationPrompt(planMarkdown);

  const capability = options?.provider?.nativeSubagents;
  if (!capability) throw new Error("Selected provider does not support native subagents");
